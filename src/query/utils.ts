function extractColumnMapping(
  ...
  tableNames.forEach((tableName) => {
    ...
    if (table) {
      table.forEach((column: any) => {
        const tableColumn = `${column.table_name}_${column.column_name}`; // table_column 형식
        const tableDotColumn = `${column.table_name}.${column.column_name}`; // table.column 형식
